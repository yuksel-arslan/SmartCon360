...
import relationshipRoutes from './modules/project/routes/relationships';
...
app.post('/projects/:id/locations/bulk', async (req, res) => {
  try {
    ...
    for (const loc of locations) {
      ...
      if (resolvedParentId) {
        const parent = created.find(c => c.id === resolvedParentId) as Record<string, unknown> | undefined ||
      ...
      const count = created.filter(c => c.parentId === resolvedParentId).length +
...
app.use(relationshipRoutes(prisma));
