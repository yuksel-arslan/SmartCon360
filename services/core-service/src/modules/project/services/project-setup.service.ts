...
export class ObsService {
  async create(projectId: string, input: CreateObsNodeInput) {
    ...
    const node = await prisma.obsNode.create({
      data: {
        ...
        uniclassCode: validated.uniclassCode,
        omniclassCode: validated.omniclassCode,
        nodeType: validated.nodeType,
        companyName: validated.companyName,
        contactName: validated.contactName,
        contactEmail: validated.contactEmail,
        contactPhone: validated.contactPhone,
        wbsResponsibility: validated.wbsResponsibility,
  ...
  async update(projectId: string, nodeId: string, input: Partial<CreateObsNodeInput>) {
    ...
    const updateData: Prisma.ObsNodeUncheckedUpdateInput = {
      updatedAt: new Date(),
    }
    if (input.code !== undefined) updateData.code = input.code
    if (input.name !== undefined) updateData.name = input.name
    if (input.description !== undefined) updateData.description = input.description
    if (input.uniclassCode !== undefined) updateData.uniclassCode = input.uniclassCode
    if (input.omniclassCode !== undefined) updateData.omniclassCode = input.omniclassCode
    ...
    if (input.wbsResponsibility !== undefined) updateData.wbsResponsibility = input.wbsResponsibility
...
export class BoqService {
  async create(projectId: string, input: CreateBoqItemInput) {
    ...
    const item = await prisma.boqItem.create({
      data: {
        projectId,
        cbsNodeId: validated.cbsNodeId,
        wbsNodeId: validated.wbsNodeId,
        ...
        sourceRowRef: validated.sourceRowRef,
  ...
  async bulkImport(projectId: string, input: BulkImportBoqInput) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const items: Prisma.BoqItemCreateManyInput[] = validated.items.map((item) => ({
        projectId,
        cbsNodeId: item.cbsNodeId,
        wbsNodeId: item.wbsNodeId,
        locationId: item.locationId,
        uniclassCode: item.uniclassCode,
        omniclassCode: item.omniclassCode,
    ...
    const doc = await prisma.projectDocument.create({
      data: {
        ...
        contractValue: validated.contractValue
          ? new Prisma.Decimal(validated.contractValue)
          : undefined,
...
export class CashFlowService {
  async generate(projectId: string, input: GenerateCashFlowInput) {
    ...
    const data: Prisma.CashFlowPeriodCreateManyInput[] = periods.map((period, index) => ({
      projectId,
