...
import { useResourceStore, EquipmentItem } from '@/stores/resourceStore';
import { useProjectStore } from '@/stores/projectStore';
import {
  Wrench, CheckCircle2, AlertTriangle, Loader2, AlertCircle,
...
const EQUIPMENT_TYPES = ['heavy', 'light', 'crane', 'vehicle', 'pump', 'generator', 'compressor', 'other'];
const EQUIPMENT_STATUSES = ['operational', 'under_maintenance', 'idle', 'breakdown', 'standby'];

export default function EquipmentPage() {
  const { activeProjectId } = useProjectStore();
  const {
    summary, equipment, loading, error,
    fetchAll, createEquipment, updateEquipment, deleteEquipment,
  } = useResourceStore();
  ...
  const operational = summary?.operationalEquipment ?? 0;
  const maintenance = summary?.underMaintenance ?? 0;

  const statusColor = (s: string) => {
    if (s === 'operational') return { bg: 'rgba(16,185,129,0.12)', text: 'var(--color-success)' };
    if (s === 'under_maintenance') return { bg: 'rgba(245,158,11,0.12)', text: 'var(--color-warning)' };
    if (s === 'breakdown') return { bg: 'rgba(239,68,68,0.12)', text: 'var(--color-danger)' };
        ...
        {!loading && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                { label: 'Total Equipment', value: totalEquipment, icon: Wrench, color: 'var(--color-accent)' },
                { label: 'Operational', value: operational, icon: CheckCircle2, color: 'var(--color-success)' },
                { label: 'Under Maintenance', value: maintenance, icon: AlertTriangle, color: 'var(--color-warning)' },
              ].map((kpi) => (
            ...
            {showForm && (
              <div className="rounded-xl border p-5 space-y-4" style={{ background: 'var(--color-bg-card)', borderColor: 'var(--color-border)' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    { key: 'name', label: 'Name', type: 'text', placeholder: 'e.g. Tower Crane TC-01' },
                    { key: 'code', label: 'Code', type: 'text', placeholder: 'e.g. EQ-001' },
                    { key: 'manufacturer', label: 'Manufacturer', type: 'text', placeholder: 'e.g. Liebherr' },
                    { key: 'model', label: 'Model', type: 'text', placeholder: 'e.g. LTM 1300' },
                    { key: 'serialNumber', label: 'Serial Number', type: 'text', placeholder: 'S/N' },
                    { key: 'capacity', label: 'Capacity', type: 'text', placeholder: 'e.g. 300t' },
                    { key: 'operatorName', label: 'Operator', type: 'text', placeholder: 'Operator name' },
            ...
            <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--color-bg-card)', borderColor: 'var(--color-border)' }}>
              <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-secondary)' }}>
                <span className="text-[11px] font-normal uppercase tracking-wide" style={{ color: 'var(--color-text-muted)' }}>Equipment ({equipment.length})</span>
                {!showForm && (
                  <button
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr>
                      {['Name', 'Code', 'Type', 'Status', 'Manufacturer', 'Utilization', 'Operator', ''].map((h) => (
                      ...
                      return (
                        <tr key={item.id}>
                          <td className="px-4 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
                            ...
                            {item.model && (
                              <div className="text-[10px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{item.model}</div>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-3 border-b text-xs" style={{ borderColor: 'var(--color-border)', color: 'var(--color-text-muted)' }}>{item.manufacturer || '--'}</td>
                          <td className="px-4 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
                            {util > 0 ? (
                          ...
                          <td className="px-4 py-3 border-b text-xs" style={{ borderColor: 'var(--color-border)', color: 'var(--color-text-muted)' }}>{item.operatorName || '--'}</td>
                          <td className="px-4 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
